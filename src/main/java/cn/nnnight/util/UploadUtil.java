...
public class UploadUtil {
	...
	public String uploadTempAvatar(MultipartFile file) {
		...
		File targetFile = new File(actualUrl + "/" + currentDateStr + "_" + file.getOriginalFilename());
		try {
		...
		return relativeUrl + "/" + currentDateStr + "_" + file.getOriginalFilename();
	}
	...
	public String uploadBlogImage(MultipartFile file) {
		...
		File targetLargeFile = new File(actualLargeUrl + "/" + currentDateStr + "_" + file.getOriginalFilename());
		File targetSmallFile = new File(actualSmallUrl + "/" + currentDateStr + "_" + file.getOriginalFilename());
		try {
		...
		return relativeLargeUrl + "/" + currentDateStr + "_" + file.getOriginalFilename();
	}
	...
	public String uploadPhotos(MultipartFile file) {
		...
		String currentDateStr = DateUtil.DateToString(new Date(), "yyyyMMddHHmmssSSS");
		String currentDay = DateUtil.DateToString(new Date(), "yyyyMMdd");
		String actualLargeUrl = actualPath + "/img/album/l/" + currentDay;
		String actualSmallUrl = actualPath + "/img/album/s/" + currentDay;
		File outLargeFile = new File(actualLargeUrl);
		File outSmallFile = new File(actualSmallUrl);
		if (!outLargeFile.exists()) {
			outLargeFile.mkdirs();
		...
		try {
			...
			returnName = currentDay + "/" + currentDateStr + "_" + file.getOriginalFilename();
		} catch (Exception e) {
			LOGGER.error("Failed to upload file. Name: {}, Exception: {}", file.getOriginalFilename(), e);
		}
		return returnName;
