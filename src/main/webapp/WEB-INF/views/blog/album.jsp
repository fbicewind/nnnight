...
<body>
							...
							<div class="album-image-box">
								<c:if test="${self }">
									...
									<ul class="image-operation-ul">
										<li><i class="fa fa-pencil-square-o" aria-hidden="true"></i>&nbsp;&nbsp;编辑</li>
										<li><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;&nbsp;&nbsp;删除</li>
									</ul>
								</c:if>
								<a href="${path }/album/detail/${album.albumId}"><img src="/static/upload/img/m/${album.coverImg }" class="full-width"></a>
								<h3 class="album-photo-count photo-count-${album.albumId }">${album.photoCount }</h3>
							</div>
						...
						<label for="publicFlag" class="col-sm-3 control-label">是否公开：</label>
							<div class="col-sm-7">
								<label class="radio-inline">
								    <input type="radio" value="1" name="publicFlag" checked="checked">是
								</label>
								<label class="radio-inline">
								    <input type="radio" value="0" name="publicFlag">否
								</label>
							</div>
						</div>
	            	</form>
	...
	<script src="${path }/static/js/album.js"></script>
	<script type="text/javascript">
		var _path = '${path}';
		var fileArr = [];
		var toAlbumId = '${albumList[0].albumId }';
